...
BarWidget {
  ...
  function formatted(date) {
    ...
    if (root.bar && root.bar.isMidnightDoll) {
    ...
    return Qt.formatDateTime(d, activeFormat.replace(/ww/g, Model.isoWeekLiteral(d.getFullYear(), d.getMonth(), d.getDate())))
  }
  ...
  WidgetButton {
    ...
    anchors.fill: parent
    bar: root.bar
    fontFamily: (root.bar && root.bar.isMidnightDoll) ? "JetBrainsMono Nerd Font" : (root.bar ? root.bar.fontFamily : Style.font.family)
